        ...
        for idxnumrep = 1:length(num_rep_arr)
            for idxrepgain = 1:length(rep_gain_arr)
                ...
                if params.CELL_REPEAT
                    ...
                    rate_ul = zeros(K-K_FWA,nbrOfRealizations);
                    for n = 1:nbrOfRealizations
                        ...
                        rate_ul(:,n) = compute_link_rates_OFDM_wi_repeater_ul(params, channel_dl, channel_dl_FWA, channel_interFWA);
                    end
                else
                    params.numCPE = 0;
                    ...
                    for n = 1:nbrOfRealizations
                        [channel_dl, channel_est_dl,channel_dl_FWA, channel_est_dl_FWA, channel_interFWA, channel_interFWA_est] = computePhysicalChannels_sub6_MIMO(params);
                        % rate_dl(:,n) = compute_link_rates_MIMO_mmse(params, channel_dl, channel_est_dl, channel_dl_FWA, channel_est_dl_FWA);
                        rate_ul(:,n) = compute_link_rates_OFDM_ul(params, channel_dl);
                    end
                end
                mean_rate_ul_cell = mean(rate_ul,2);
                if (quantile(mean_rate_ul_cell,params.loss_pc_cell)>=params.r_min_cell)
                    params.Band = max(params.Band*(1-params.r_min_cell/quantile(mean_rate_ul_cell,params.loss_pc_cell)),0);
                    if (params.Band > 0)
                        mean_rate_ul_cell = mean_rate_ul_cell*params.r_min_cell/quantile(mean_rate_ul_cell,params.loss_pc_cell);
                    end
                else
                    params.Band = 0;
                end
                ...
                for idxrmin = 1:length(r_min_arr)
                    ...
                    rateFolder = strcat(dataFolder,'/FWA_multi_cell_repeater_fixed_FWA_cf_var_rep_num_gain_ul');
                    ...
                    result_string = strcat('/results_numFWA_',num2str(params.Lmax), 'Lmax_', num2str(numCPE_all),...
                        ...
                        num2str(params.num_repeater_per_cpe),'num_repeater', ...
                        aID);
                    ...
                    lambda_UE(idxUEDensity),deployRange,params.r_min_cell,params.r_min_FWA,params.num_repeater_per_cpe,params.repeat_gain,K_FWA_max,Band,Band_FWA, sum(mean_rate_ul_cell)/(Band - Band_FWA), sum_FWA_rate/(Band_FWA*K_FWA_max));
                    fclose(fileID);
                end
            end
        end
