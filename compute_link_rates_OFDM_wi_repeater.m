...
for k = 1:K-K_FWA
    for n = 1:N_UE
        ...
        ds_base = p_d*abs(eff_channel'*eff_channel./norm(eff_channel))^2;
        ...
        for nn = 1:N_UE
            if (nn~=n)
                ...
                if (norm(nn_eff_channel,'fro') < norm(eff_channel,'fro'))
                    msi_base = p_d*abs(eff_channel'*nn_eff_channel./norm(nn_eff_channel))^2;
                ...
                if ismember(k,I_idxs)
                    ...
                    mci_base = p_d*abs(eff_channel'*mci_eff_channel./norm(mci_eff_channel))^2;
        ...
        if ismember(k,I_idxs)
            ...
            mci_base2 = p_d*abs(eff_channel'*mci_eff_channel./norm(mci_eff_channel))^2;
