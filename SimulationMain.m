...
for idxUEDensity = 1:length(lambda_UE)
    ...
    for idxBSDensity = 1:length(lambda_BS)
        ...
        params.numSC = ceil(lambda_SC(idxBSDensity)*pi*(params.deployRange/1000)^2);
        params.Rsc = params.deployRange*sqrt(rand(params.numSC,1));
        ...
        result_string = strcat('/results_numFWA_',num2str(params.SC), 'SC_', num2str(numCPE_all),...
            ...
            'lambdaUE_', num2str(deployRange),'deployRange_',aID);
        ...
        'deployRange,','max_pow_fac,','max_FWA,','max_cell_util,','max_FWA_util\n'];
        ...
        formatSpec = '%d,%d,%d,%d,%f,%d,%f,%.16f,%.16f\n';
        fprintf(fileID,formatSpec,lambda_BS(idxBSDensity),lambda_SC(idxBSDensity),numCPE_all, ...
        lambda_UE(idxUEDensity),deployRange,p_fac_max,K_FWA_max,max_cell_util,max_FWA_util);
        fclose(fileID);
    end
end
tEnd = toc(tStart);
