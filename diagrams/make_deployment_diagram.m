%%DEPLOYMENT DIAGRAM GENERATOR: 2D top-down device map (zones, CPEs by class, active UEs by mobility class)
...
%% Simulation FR1 setup
    ...
    for s = 1:M_sites
        ...
        plot(cx+params.aptRadius*cos(th),  cy+params.aptRadius*sin(th),  '-',  'Color',[.55 .55 .55],'LineWidth',0.7);
        plot(cx+params.homeRadius*cos(th), cy+params.homeRadius*sin(th), '--', 'Color',[.55 .55 .55],'LineWidth',0.7);
        plot(cx+params.deployRange*cos(th),cy+params.deployRange*sin(th),':',  'Color',[.55 .55 .55],'LineWidth',0.9);
    end
    plot(params.mallRadius*cos(th), params.mallRadius*sin(th), '-', 'Color',[.3 .3 .3],'LineWidth',0.8); %shared mall
    for k2 = 1:numel(opC) %office parks
        plot(real(opC(k2))+params.opRadius*cos(th), imag(opC(k2))+params.opRadius*sin(th), '-', 'Color',[.3 .3 .3],'LineWidth',0.8);
    end
    %FWA CPEs by zone
    zc = params.cpe_zone;
    h1 = plot(CPE_locations(zc==1,1), CPE_locations(zc==1,2), 's', 'Color',[0 .45 .74],'MarkerFaceColor',[0 .45 .74],'MarkerSize',5);
    h2 = plot(CPE_locations(zc==2,1), CPE_locations(zc==2,2), 's', 'Color',[0 .45 .74],'MarkerSize',4);
    h3 = plot(CPE_locations(zc==3,1), CPE_locations(zc==3,2), 'd', 'Color',[.49 .18 .56],'MarkerFaceColor',[.49 .18 .56],'MarkerSize',6);
    h4 = plot(CPE_locations(zc==4,1), CPE_locations(zc==4,2), 'd', 'Color',[.85 .33 .1],'MarkerFaceColor',[.85 .33 .1],'MarkerSize',6);
    %active cellular UEs by class
    fast = v_move_ue > 5; %in-car (40 km/h)
    ped  = ~fast & ~isIndoorUE;
    ind  = ~fast &  isIndoorUE;
    h5 = plot(UE_locations(ped,1),  UE_locations(ped,2),  'o', 'Color',[.47 .67 .19],'MarkerSize',4,'LineWidth',1);
    h6 = plot(UE_locations(ind,1),  UE_locations(ind,2),  '.', 'Color',[.47 .67 .19],'MarkerSize',10);
    h7 = plot(UE_locations(fast,1), UE_locations(fast,2), 'x', 'Color',[.64 .08 .18],'MarkerSize',7,'LineWidth',1.3);
    %macro sites
    h8 = plot(siteLocations(:,1), siteLocations(:,2), '^', 'Color','k','MarkerFaceColor','k','MarkerSize',11);
    legend([h8 h1 h2 h3 h4 h5 h6 h7], {'gNB site (3 sectors)', ...
        sprintf('CPE, apartment block (%d)',sum(zc==1)), sprintf('CPE, single-family home (%d)',sum(zc==2)), ...
        sprintf('CPE, mall unit (%d)',sum(zc==3)), sprintf('CPE, office park (%d)',sum(zc==4)), ...
